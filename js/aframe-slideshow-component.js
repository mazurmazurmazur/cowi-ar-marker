      ...
      AFRAME.registerComponent("aframe-slideshow", {
        ...
        initVariables: function() {
          ...
          if (this.data.startpos) {
            ...
            this.startpos.x =
              -((this.data.nbColumns - 1) / 2) * this.data.distBetweenSlides;
            this.startpos.y = this.data.slideYPos;
            this.startpos.z =
              ((this.nbChildren / this.data.nbColumns - 1) / 2) *
              this.data.distBetweenSlides;
          }
        ...
        addListeners: function() {
          ...
          document
            ...
            .addEventListener("exit-vr", function() {
              document
                .querySelector("#camParent")
                .object3D.position.copy(that.oldCamParentPos);
            });
          document
            .querySelector("a-scene")
            .addEventListener("enter-vr", function() {
              that.oldCamParentPos.copy(
        ...
        initSlides: function() {
          ...
          var slidesVideo = this.data.vidSlidesIndex
            .split(",")
            .map(function(obj) {
              return parseInt(obj, 10);
            });
          var slidesAnimated = this.data.animSlidesIndex
            .split(",")
            .map(function(obj) {
              return parseInt(obj, 10);
            });
          ...
          for (var i = 1, len = this.data.nbslides; i <= len; ++i) {
            ...
            if (slidesVideo.indexOf(i) !== -1) {
              ...
              entity.setAttribute(
                "aframe-slideshow-slide",
                "src: " +
                  this.data.folder +
                  this.data.namingConv.replace("%num%", i) +
                  "." +
                  this.data.videoExtension +
                  "; type: video;" +
                  animate
              );
            } else {
        ...
        onSlideshowReady: function() {
          document
            ...
            .classList.remove("make-container--visible");
        ...
        goToSlide: function(index, skipAnimation) {
          ...
          var camera = document.querySelector("a-scene").camera,
            ...
            geomSize = document.querySelector("#" + slideChild.id).components[
              "aframe-slideshow-slide"
            ].geomWidth,
            camFov = camera.fov,
          ...
          var posZ =
            geomSize / 2 / Math.tan((camFov * Math.PI) / 180 / 2) / canvasRatio;

          var newPos = new THREE.Vector3(
            slideChild.object3D.position.x,
            0,
            slideChild.object3D.position.z + posZ
          );
          if (
            slideChild.components["aframe-slideshow-slide"].data
              .animTransition &&
            !skipAnimation
          ) {
            var pointArray = [];
            ...
            pointArray.push(
              newPos
                .clone()
                .sub(camParent.position)
                .divideScalar(2)
                .add(camParent.position)
            ...
            document
              .querySelector("#acamera")
              .components["look-controls"].el.setAttribute(
                "rotation",
                "0, 0, 0"
              );
          }
        ...
        tick: function(time, timeDelta) {
          if (this.deltaTransition < 1) {
            ...
            camParent.position.copy(
              this.cameraPath.getPointAt(this.deltaTransition)
            );
